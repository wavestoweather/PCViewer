...
void CompressionWorkbench::draw() 
{
    ...
    if(ImGui::Begin("CompresssionWorkbench", &active)){
        ...
        if(ImGui::Button("Cluster ND hierarchy")){
            compression::createNDHierarchy(_outputFolder, _loader.get(), compression::CachingMethod::Bundled, _startCluster, _clusterMultiplicator, _dimensionality, _levels, _maxWorkingMemory, _amtOfThreads);
        }
    }
    ImGui::End();
}
