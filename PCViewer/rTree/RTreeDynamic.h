...
/*
  ...
  The original code can be found in RTreeDynamic.h
  The code here is adopted to be able to have dimension sizes set at runtime
*/
...
#define RTREE_QUAL RTreeDynamic<DATATYPE, ELEMTYPE, ELEMTYPEREAL, TMAXNODES, TMINNODES>
...
/// \class RTreeDynamic
/// Implementation of RTreeDynamic, a multidimensional bounding rectangle tree.
/// Example usage: For a 3-dimensional tree use RTreeDynamic<Object*, float, 3> myTree;
///
...
public:
  ...
  RTreeDynamic(int numDims);
  RTreeDynamic(const RTreeDynamic& other);
  virtual ~RTreeDynamic();
  ...
  private:
    ...
    friend class RTreeDynamic; // Allow hiding of non-public functions while allowing manipulation by logical owner
  };
...
RTREE_TEMPLATE
RTREE_QUAL::RTreeDynamic(int numDims):NUMDIMS(numDims)
{
...
bool RTREE_QUAL::LoadRec(Node* a_node, RTFileStream& a_stream)
{
  ...
  if(a_node->IsInternalNode())  // not a leaf node
  {
    for(int index = 0; index < a_node->m_count; ++index)
    {
      ...
      curBranch->m_rect.m_min.resize(NUMDIMS);
      curBranch->m_rect.m_max.resize(NUMDIMS);
...
void RTREE_QUAL::CopyRec(Node* current, Node* other)
{
  ...
  if(current->IsInternalNode())  // not a leaf node
  {
    for(int index = 0; index < current->m_count; ++index)
    {
      ...
      currentBranch->m_rect.m_min.resize(NUMDIMS);
      currentBranch->m_rect.m_max.resize(NUMDIMS);
