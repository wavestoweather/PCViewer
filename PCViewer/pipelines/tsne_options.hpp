...
namespace pipelines
{
    ...
    public:
        ...
        tsne_options() {}
        tsne_options(float *return_data, float *points, int num_points, int num_dims) : return_data(return_data), points(points), num_points(num_points),
                                                                                   num_dims(num_dims) { this->random_seed = time(NULL); }
        tsne_options(float *points, int num_points, int num_dims,
    ...
    public:
        ...
        GpuOptions(int device)
        {
            ...
            this->warp_size = globals::vk_context.subgroup_properties.subgroupSize;
            // if (this->warp_size != 32)
            // {
            //     std::cerr << "E: Device warp size not supported: " << this->warp_size << std::endl;
            //     exit(1);
            // }
            this->sm_count = 100;//globals::vk_context.subgroup_properties.subgroupCount;

            // Set some per-architecture structures
            if (true)//device_properties.major >= 7)
            { // TURING
            ...
            else if (false)//device_properties.major >= 6)
            { // PASCAL/VOLTA
                this->integration_kernel_threads = 1024;
                this->integration_kernel_factor = 1;
                this->repulsive_kernel_threads = 256;
            ...
            else if (false)//device_properties.major >= 5)
            { // MAXWELL
                this->integration_kernel_threads = 1024;
                this->integration_kernel_factor = 1;
                this->repulsive_kernel_threads = 256;
            ...
            else if (false)//device_properties.major >= 3)
            { // KEPLER
