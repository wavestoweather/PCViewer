...
void brusher::brush(const brush_info& info) 
{
    ...
    pc.index_buffer_address = tl.name == structures::default_templatelist_name ? VkDeviceAddress{}: util::vk::get_buffer_address(tl.gpu_indices);
    pc.local_global_brush_combine = static_cast<uint32_t>(info.brush_comb);
    pc.line_count = ds.gpu_stream_infos ? ds.gpu_stream_infos->block_size: ds.data_size;
    ...
    vkCmdPushConstants(_command_buffer, _brushing_pipeline_layout, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, sizeof(pc), &pc);
    uint32_t dispatch_x = (pc.line_count + shader_local_size - 1) / shader_local_size;
