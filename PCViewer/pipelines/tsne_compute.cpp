...
void tsne_compute::calculate(const tsne_calculate_info& info){
    ...
    tsne_compute_info record_info{};
    record_info.dst_address = info.src_address;
    record_info.src_address = info.dst_address;
    record_info.tmp_address = info.tmp_address;
    record_info.datapoint_count = as<uint32_t>(info.tsne_options.num_points);
    record_info.dimension_count = as<uint32_t>(info.tsne_options.num_dims);
    record_info.tmp_memory_infos = info.memory_info;
