...
struct ImVec4
{
    ...
    float  operator[] (size_t idx) const                      { IM_ASSERT(idx <= 3); return (&x)[idx]; }    // We very rarely use this [] operator, the assert overhead is fine.
    float& operator[] (size_t idx)                            { IM_ASSERT(idx <= 3); return (&x)[idx]; }    // We very rarely use this [] operator, the assert overhead is fine.
...
namespace ImGui
{
    ...
    IMGUI_API int			PlotHistogramVertical(const char* label, const float* values, int values_count, int values_offset = 0, const char* overlay_text = NULL, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0, 0), int stride = sizeof(float));
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_PreviewInForeground          = 1 << 13,  // Render the preview window to the foreground drawlist
};
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddPie(const ImVec2& center, float radius, ImU32 col, float percentage, int num_segments = -1, float border = 3.0f);
    IMGUI_API int   AddPie(const ImVec2& center, float radius, ImU32* cols, float* percentages, int pieCount, int num_segments = -1, float border = 3.0f, int highlight_hover = -1);
...
namespace ImGui
{
    ...
    static inline bool  SliderDouble(const char* label, double* v, double v_min, double v_max, const char* format, float power)             { return SliderScalar(label, ImGuiDataType_Double, v, &v_min, &v_max, format, power); }
