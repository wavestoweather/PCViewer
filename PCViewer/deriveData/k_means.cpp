...
void k_means::run(const float_column_views& input, float_column_views& output, const k_means_settings_t& settings){
    ...
    for(size_t i: util::i_range(input.size() - 1))
    ...
    case init_method_t::forgy:{
        ...
        for(auto& center: cluster_centers){
            ...
            for(size_t i: util::size_range(input)) center[i] = equal_layout ? input[i].cols[0][index]: input[i](index, 0);
        }
        break;}
    case init_method_t::uniform_random:{
        std::uniform_int_distribution<int> distribution(0, settings.cluster_count - 1);
        for(size_t i: util::i_range(data_size)) output[0].cols[0][i] = as<float>(distribution(random_engine));
    ...
    case init_method_t::normal_random:{
        ...
        for(size_t d: util::size_range(input)){
            ...
            for(size_t i: util::size_range(cluster_centers)) cluster_centers[i][d] = as<float>(distribution(random_engine));
        }
        break;}
    case init_method_t::plus_plus:{
        ...
        size_t index = distribution(random_engine);
        for(size_t i: util::size_range(input)) cluster_centers[0][i] = equal_layout ? input[i].cols[0][index]: input[i](index, 0);
        ...
        for(int i: util::i_range(1, settings.cluster_count)){
            ...
            if(probability_sum <= 0)
                continue;
            float rand = as<float>(f_distribution(random_engine) * probability_sum);
            for(size_t j: util::i_range(data_size)){
