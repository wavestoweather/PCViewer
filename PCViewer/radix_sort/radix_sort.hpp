...
void RadixSortMSDTransform(RadixableType* data, size_t N, const transformer& transform, size_t bit = (8 * sizeof(std::declval<transformer>()({}))) - 1) {
  ...
  int right = N;
  while (left < right) {
    size_t left_bit = ((transform(data[left]) >> bit) & 1);
    ...
    size_t right_bit = ((transform(data[right - 1]) >> bit) & 1);
    if (right_bit == 1) {
      --right;
      continue;
    }
   ...
   */
  if (bit > 0) {
    RadixSortMSDTransform(data, left, transform, bit - 1);
    RadixSortMSDTransform(data + left, N - left, transform, bit - 1);
  }
}
}
